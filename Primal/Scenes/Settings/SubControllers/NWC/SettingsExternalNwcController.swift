...
private extension SettingsExternalNwcController {
    func setup() {
        ...
        let midStack = UIStackView(axis: .vertical, [
            UILabel("An external app “\(params.appName)” is requesting access to your Crays Wallet. If you wish to allow this, click “Give Wallet Access” below.", color: .foreground, font: .appFont(withSize: 16, weight: .regular), multiline: true), SpacerView(height: 24),
            budget, SpacerView(height: 20),
            UILabel("You can revoke access at any time in your Crays Wallet settings.", color: .foreground4, font: .appFont(withSize: 16, weight: .regular), multiline: true)
        ])
