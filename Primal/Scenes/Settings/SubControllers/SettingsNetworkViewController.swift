...
private extension SettingsNetworkViewController {
    func setup() {
        ...
        let regularConnection = SettingsNetworkStatusView(title: Connection.regular.socketURL.absoluteString.replacingOccurrences(of: "primal", with: "crays"))
        ...
        Connection.regular.$socketURL.receive(on: DispatchQueue.main).sink { url in
            regularConnection.title = url.absoluteString.replacingOccurrences(of: "primal", with: "crays")
        }
        .store(in: &cancellables)
    ...
    func relayConnectionView(_ connection: RelayConnection, last: Bool) -> UIView {
        let view = SettingsNetworkStatusListView(title: connection.identity.replacingOccurrences(of: "primal", with: "crays"), onDelete: { [weak self] in
            let alert = UIAlertController(title: "Are you sure?", message: "Do you want to delete this relay?\n\(connection.identity.replacingOccurrences(of: "primal", with: "crays"))", preferredStyle: .alert)
            alert.addAction(.init(title: "OK", style: .destructive) { _ in
