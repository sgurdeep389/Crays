...
final class WalletHomeViewController: UIViewController, Themeable {
    ...
    let comingSoonView:UIImageView = UIImageView(image: ._0005720ComingSoonPage550)
...
private extension WalletHomeViewController {
    func setup() {
        title = "Wallet"
        comingSoonView.contentMode = .scaleAspectFit
        let stack = UIStackView(axis: .vertical, [navBar, table,comingSoonView])
        ...
        stack.pinToSuperview(edges: .horizontal).pinToSuperview(edges: .top, safeArea: true).pinToSuperview(edges: .bottom, padding: 200)
        ...
        .sink { [weak self] hasWallet, isLoading, transactions, shouldShowBuySats in
            ...
            guard LoginManager.instance.method() == .nsec else {
                tableData = [firstSection, Section(cells: [.error("Crays is in read only mode because you are signed in via your public key. To enable all options, please sign in with your private key, starting with 'nsec...")])]
                return
            }
