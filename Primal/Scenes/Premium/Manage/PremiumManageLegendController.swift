...
class PremiumManageLegendController: UIViewController {
    ...
    let infoLabel = UILabel("Legend cards contain a personalized shoutout from\nCrays to all our legends.", color: .foreground3, font: .appFont(withSize: 12, weight: .regular), multiline: true)
...
private extension PremiumManageLegendController {
    func setup() {
        ...
        titleView.titleLabel.text = state.cohort_1.replacingOccurrences(of: "Primal", with: "Crays")
        ...
        if hasShoutoutEdits {
            ...
            infoLabel.text = "Your edits have been sent to Crays for review. The updated content should be live soon."
        } else {
    ...
    func setupInputView() {
        ...
        sendButton.addAction(.init(handler: { [weak self] _ in
            ...
            self?.infoLabel.text = "Your edits have been sent to Crays for review. The updated content should be live soon."
            self?.closeInputView()
        }), for: .touchUpInside)
    }
