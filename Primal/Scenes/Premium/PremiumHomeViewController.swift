...
private extension PremiumHomeViewController {
    ...
    func userStackView() -> UIView? {
        ...
        if state.isExpired {
            ...
            let label = UILabel("Hey there! You are an early Crays user who interacted with our team, so we gave you 6 months of Crays Premium for free. ♥️🫂", color: .foreground3, font: .appFont(withSize: 14, weight: .regular))
    ...
    func actionLabelString() -> NSAttributedString {
        ...
        let strings: (String, String?) = {
            ...
            if state.isExpired {
                return ("Your Crays Premium subscription has expired.\nYou can renew it below:", nil)
            }
            return ("Want to get more out of Crays?\nCheck out ", "Crays Pro")
        }()
