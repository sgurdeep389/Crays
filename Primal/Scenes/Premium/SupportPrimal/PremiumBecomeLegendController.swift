...
class PremiumBecomeLegendController: UIViewController {
    override func viewDidLoad() {
        ...
        let descStack = UIStackView(axis: .vertical, [
            ...
            UILabel("contribution to Crays.",color: .foreground, font: .appFont(withSize: 15, weight: .regular))
        ])
        ...
        let privateBetaTitle = BecomeLegendTitledParagraph(title: "Way More Storage", paragraph: "Get 100GB of Crays Premium media storage.")
        ...
        let infoStack = UIStackView(axis: .vertical, [
            BecomeLegendTitledParagraph(title: "Forever Premium", paragraph: "Crays Premium subscription never expires for legends."),
            privateBetaTitle,
            legendaryTitle,
        ])
        ...
        action.addAction(.init(handler: { [weak self] _ in
            guard let state = WalletManager.instance.premiumState else {
                ...
                self?.show(PremiumSearchNameController(title: "Find Crays Name", callback: { name in
