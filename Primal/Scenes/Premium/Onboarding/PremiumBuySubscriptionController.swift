...
private extension PremiumBuySubscriptionController {
    func setup() {
        ...
        switch kind {
        case .premium:
            learnMoreButton.configuration = .accent("Learn about Crays Premium", font: .appFont(withSize: 16, weight: .regular))
        case .pro:
            learnMoreButton.configuration = .coloredButton("Learn about Crays Pro", color: .pro)
        }
        ...
        table.addressRow.infoLabel.text = pickedName + "@crays.net"
        table.lightningRow.infoLabel.text = table.addressRow.infoLabel.text
        table.profileRow.infoLabel.text = "crays.net/" + pickedName
        ...
        InAppPurchaseManager.shared.purchase(product: product) { [weak self] result in
            ...
            case .success(let verification):
                ...
                case .unverified(_, let error):
                    self?.loader.isHidden = true
                    RootViewController.instance.view.isUserInteractionEnabled = true
                    RootViewController.instance.showErrorMessage(title: "The Crays Premium subscription purchase failed. Please try again.", error.localizedDescription)
                    print("Purchase error: \(error)")
                }
            case nil:
                self?.loader.isHidden = true
                RootViewController.instance.view.isUserInteractionEnabled = true
                RootViewController.instance.showErrorMessage(title: "The Crays Premium subscription purchase failed. Please try again.", "Check your apple pay settings")
            case .userCancelled:
                self?.loader.isHidden = true
                RootViewController.instance.view.isUserInteractionEnabled = true
            case .pending:
                self?.loader.isHidden = true
                RootViewController.instance.view.isUserInteractionEnabled = true
                RootViewController.instance.showErrorMessage(title: "Your Crays Premium subscription purchase is pending. Once it completes, the Premium features will be enabled.")
            }
        }
    }
    ...
    func userStackView() -> UIView? {
        ...
        if state == .onboardingFinish || state == .upgradeToPro {
            ...
            userStack.addArrangedSubview(UILabel(
                state == .upgradeToPro ? "Your Legend status is one click away" : "Your Crays Name is available!",
