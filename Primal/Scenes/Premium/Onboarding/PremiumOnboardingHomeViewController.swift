...
private extension PremiumOnboardingHomeViewController {
    ...
    var proInfoItems: [String] { [
        ...
        "Crays Studio",
        "Legend Status on Crays",
        "100GB media storage",
        "10GB max file size"
    ] }
    
    func setup() {
        let introLabel = ThemeableLabel("Upgrade your Crays experience today.", textColor: { .foreground3 }, font: .appFont(withSize: 18, weight: .regular))
        ...
        if let premium = WalletManager.instance.premiumState, !premium.isExpired {
            topStack.addArrangedSubview(UILabel("Crays Pro", color: .foreground, font: .appFont(withSize: 36, weight: .bold)))
            ...
            if RootViewController.instance.view.frame.height > 700 {
                [
                    UILabel("Crays", color: .foreground, font: .appFont(withSize: 36, weight: .bold)),
        ...
        $isViewingPro.removeDuplicates().sink { isPro in
            UIView.transition(with: botStack, duration: 0.2, options: .transitionCrossDissolve) {
                learnAboutButton.configuration = isPro ? .coloredButton("Learn about Crays Pro", color: .pro) : .coloredButton("Learn about Crays Premium", color: .accent)
        ...
        premiumInfoView.actionButton.addAction(.init(handler: { [weak self] _ in
            ...
            nav.pushViewController(PremiumSearchNameController(title: "Find Crays Name", callback: { name in
        ...
        proInfoView.actionButton.addAction(.init(handler: { [weak self] _ in
            ...
            nav.pushViewController(PremiumSearchNameController(title: "Find Crays Name", buttonTint: .pro, callback: { name in
...
class LearnAboutPremiumView: UIView {
    ...
    init(title: String, tint: UIColor, items: [String], hideFirstCheckbox: Bool = false) {
        ...
        let primalLabel = UILabel("Crays", color: .foreground, font: .appFont(withSize: 24, weight: .bold))
