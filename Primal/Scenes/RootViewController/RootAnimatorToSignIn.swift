...
//                }
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + .milliseconds(16000 / speedInt)) {
                ...
                let views = [onboarding.titleLabel, onboarding.signupButton, onboarding.signinButton, onboarding.redeemCodeButton, onboarding.termsBothLines]
                views.forEach {
                    $0.alpha = 0
                    $0.transform = .init(translationX: 0, y: 100)
                }
                onboarding.titleLabel.transform = .init(scaleX: 0.66, y: 0.66)
                
                CATransaction.begin()
                CATransaction.setAnimationTimingFunction(.signinEaseOut)
                
                UIView.animate(withDuration: 17 / speed) {
                    onboarding.titleLabel.alpha = 1
                    onboarding.titleLabel.transform = .identity
