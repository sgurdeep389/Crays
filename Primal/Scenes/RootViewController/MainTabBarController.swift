...
enum MainTab: String {
    case home, reads, wallet, explore,nearbyChat
...
final class MainTabBarController: UIViewController, Themeable {
    ...
    lazy var wallet = MainNavigationController(rootViewController: MenuContainerController(child: UnderDevVC()))
    lazy var nearbychat = MainNavigationController(rootViewController: MenuContainerController(child: MessagesViewController(titleLbl: "Messages")))
    ...
    private let circleBorderView = ThemeableView().constrainToSize(40).setTheme {
        $0.backgroundColor = .background
        $0.layer.borderColor = UIColor.background3.cgColor
    }
    private let navigationBorder = UIView().constrainToSize(height: 1)
    private lazy var circleWalletButton = ThemeableButton().constrainToSize(40).setTheme { [weak self] in
        ...
        $0.setImage(isWalletSelected ? UIImage(named: "output-onlinepngtools") : UIImage(named: "output-onlinepngtools"), for: .normal)
    }
    ...
    private let tabs: [MainTab] = [.home, .reads, .wallet, .explore, .nearbyChat]
    ...
    func updateTheme() {
        ...
        if updateChildren {
            [home, reads, wallet, explore,nearbychat].forEach {
...
private extension MainTabBarController {
    func setup() {
        ...
        if let userDefaults = UserDefaults(suiteName: "group.chat.Crays") {
                    ...
                    let (vc, tab) : (UIViewController?, MainTab?) = {
                        ...
                        case .search(let text):
                            return (SearchNoteFeedController(feed: FeedManager(newFeed: PrimalFeed(
                                ...
                                description: "Crays search results",
                                feedkind: "search",
                                enabled: true
                            ))), .home)
                        case .tab(let mainTab):
                            return (nil, mainTab)
                        case .messages:
                            return (MessagesViewController(titleLbl: "Messages"), .home)
                        case .bookmarks:
    ...
    func addCircleWalletButton() {
        ...
        circleBorderView.pinToSuperview(edges: .top, padding: 0).centerToSuperview(axis: .horizontal)
        circleBorderView.layer.borderWidth = 1
        circleBorderView.layer.cornerRadius = 32
        
        let frontCover = ThemeableView().constrainToSize(35).setTheme { $0.backgroundColor = .background }
        frontCover.layer.cornerRadius = 31
        buttonStackParent.addSubview(frontCover)
        frontCover.pinToSuperview(edges: .top, padding: 0).centerToSuperview(axis: .horizontal)
        
        circleWalletButton.layer.cornerRadius = 26
        buttonStackParent.addSubview(circleWalletButton)
        circleWalletButton.pinToSuperview(edges: .top, padding: 8).centerToSuperview(axis: .horizontal)
