...
private extension ProfileShowQRController {
    ...
    func update(_ user: ParsedUser) {
        ...
        userInfo.address.text = " \(user.data.nip05.replacingOccurrences(of: "primal", with: "crays"))" // we add whitespace in case nip05 is empty, the label shouldn't collapse to 0 height
    ...
    func updateTabs(_ user: ParsedUser) {
        ...
        if !user.data.lud16.isEmpty {
            options.append(.init(name: "LIGHTNING ADDRESS", text: user.data.lud16.replacingOccurrences(of: "primal", with: "crays"), qrCode: "lightning:\(user.data.lud16)", logo: UIImage(named: "qr-lightning")))
        }
        ...
        for ((button, selectedButton), option) in zip(buttons, options) {
            ...
            button.addAction(.init(handler: { [weak self] _ in
                ...
                userInfo.address.text = " \(isFirst ? user.data.nip05 : user.data.lud16) ".replacingOccurrences(of: "primal", with: "crays")
