        ...
        let (actionText, description) = {
            guard IdentityManager.instance.userHexPubkey.isEmpty else {
                if WalletManager.instance.userHasWallet != true {
                    return ("Activate Wallet", "To redeem your code, activate your Crays Wallet")
                }
                return ("Redeem Code", "")
            }
            return ("Onboard to Crays", "To redeem your code, onboard to Crays by creating your Nostr account and activating your Crays Wallet.")
        }()
...
class CodePreviewPremiumView: UIStackView {
    init(months: Int) {
        ...
        let firstStack = UIStackView([
            ...
            UILabel("Crays Premium", color: .gray11, font: .appFont(withSize: 24, weight: .semibold))
        ])
