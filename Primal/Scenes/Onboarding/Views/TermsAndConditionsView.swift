...
final class TermsAndConditionsView: UIStackView, Themeable {
    ...
    static let termsURL = URL(string: "https://crays-terms-of-services.vercel.app")!
    static let privacyURL = URL(string: "https://crays-policy.vercel.app")!
    ...
    init(whiteOverride: Bool = false) {
        ...
        termsButton.addAction(.init(handler: { _ in
            let nv = UINavigationController(rootViewController: WebViewController(urlString: Self.termsURL.absoluteString, title: "Terms of Service"))
            nv.modalPresentationStyle = .overFullScreen
            RootViewController.instance.show(nv, sender: nil)


        }), for: .touchUpInside)
        
        privacyButton.addAction(.init(handler: { _ in
            let nv = UINavigationController(rootViewController: WebViewController(urlString: TermsAndConditionsView.privacyURL.absoluteString, title: "Privacy Policy"))
            nv.modalPresentationStyle = .overFullScreen
            RootViewController.instance.show(nv, sender: nil)
        }), for: .touchUpInside)
