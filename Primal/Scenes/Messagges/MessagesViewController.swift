...
final class MessagesViewController: UIViewController, Themeable {
    ...
    private let nearByChat = UIButton()
    ...
    func updateTheme() {
        ...
        nearByChat.setTitleColor(.foreground, for: .normal)
    ...
    func updateButtonFonts() {
        ...
        nearByChat.titleLabel?.font = .appFont(withSize: 14, weight: .bold)
...
private extension MessagesViewController {
    func setup() {
        ...
        let hstack = UIStackView([SpacerView(width: 12), nearByChat,SpacerView(width: 25),followsButton, SpacerView(width: 25), otherButton,SpacerView(width: 25), UIView(), markAllRead, SpacerView(width: 12)])
        ...
        nearByChat.setTitle("CRAYS MESH", for: .normal)
        ...
        nearByChat.addAction(.init(handler: { [weak self] _ in
            self?.show(NearByChat(), sender: nil)
        }), for: .touchUpInside)
