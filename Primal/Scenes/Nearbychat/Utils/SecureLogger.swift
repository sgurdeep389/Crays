...
class SecureLogger {
    ...
    private static let subsystem = "chat.Crays"
    ...
    private static let cacheQueue = DispatchQueue(label: "chat.Crays.securelogger.cache", attributes: .concurrent)
    ...
    /// Minimum level that will be logged. Defaults to .info. Override via env Crays_LOG_LEVEL.
    private static let minimumLevel: LogLevel = {
        let env = ProcessInfo.processInfo.environment["Crays_LOG_LEVEL"]?.lowercased()
        switch env {
        case "debug": return .debug
