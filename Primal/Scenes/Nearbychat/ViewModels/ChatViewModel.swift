...
/// # ChatViewModel
///
/// The central business logic and state management component for Crays.
...
/// SwiftUI views and the underlying Crays services. It manages:
...
/// - **CraysDelegate**: Receives messages and events from BLEService
...
/// Manages the application state and business logic for Crays.
/// Acts as the primary coordinator between UI components and backend services,
/// implementing the CraysDelegate protocol to handle network events.
class ChatViewModel: ObservableObject, CraysDelegate {
    ...
    @MainActor
    private func normalizedSenderKey(for message: CraysMessage) -> String {
    ...
    @Published var messages: [CraysMessage] = []
    ...
    @Published var allPeers: [CraysPeer] = []
    var privateChats: [String: [CraysMessage]] { 
    ...
    private var peerIndex: [String: CraysPeer] = [:]
    ...
    private let nicknameKey = "Crays.nickname"
    ...
    private var publicBuffer: [CraysMessage] = []
        ...
        Task { @MainActor in
            ...
            let cancellable = unifiedPeerService.$peers
                ...
                .sink { [weak self] peers in
                    ...
                    var uniquePeers: [String: CraysPeer] = [:]
                    for peer in peers {
    ...
    private func resubscribeCurrentGeohash() {
        ...
        do {
            ...
            NostrRelayManager.shared.subscribe(filter: dmFilter, id: dmSub) { [weak self] giftWrap in
                ...
                guard content.hasPrefix("Crays1:") else { return }
                guard let packetData = Self.base64URLDecode(String(content.dropFirst("Crays1:".count))),
                      let packet = CraysPacket.from(packetData) else { return }
    ...
    /// Sends a message through the Crays network.
    ...
    func updateBluetoothState(_ state: CBManagerState) {
        ...
        switch state {
        case .poweredOff:
            bluetoothAlertMessage = "Bluetooth is turned off. Please turn on Bluetooth in Settings to use Crays."
            showBluetoothAlert = true
        case .unauthorized:
            bluetoothAlertMessage = "Crays needs Bluetooth permission to connect with nearby devices. Please enable Bluetooth access in Settings."
            showBluetoothAlert = true
        case .unsupported:
            bluetoothAlertMessage = "This device does not support Bluetooth. Crays requires Bluetooth to function."
            showBluetoothAlert = true
        case .poweredOn:
                ...
                if let nostrMessages = privateChats[noiseKeyHex], !nostrMessages.isEmpty {
                    ...
                    for message in nostrMessages {
                        if !existingMessageIds.contains(message.id) {
                            ...
                            let updatedMessage = CraysMessage(
                                id: message.id,
    ...
    @objc private func handleNostrMessage(_ notification: Notification) {
        guard let message = notification.userInfo?["message"] as? CraysMessage else { return }
    ...
    @objc private func userDidTakeScreenshot() {
        ...
        if let peerID = selectedPrivateChatPeer {
            ...
            let localNotification = CraysMessage(
                sender: "system",
    ...
    @MainActor
    func getPrivateChatMessages(for peerID: String) -> [CraysMessage] {
        var combined: [CraysMessage] = []
        ...
        var bestByID: [String: CraysMessage] = [:]
        for msg in combined {
    ...
    func panicClearAllData() {
        ...
        userDefaults.removeObject(forKey: "Crays.noiseIdentityKey")
        userDefaults.removeObject(forKey: "Crays.messageRetentionKey")
    ...
    func getSenderColor(for message: CraysMessage, colorScheme: ColorScheme) -> Color {
        let isDark = colorScheme == .dark
        let primaryColor = isDark ? Color.white : .black
        
        return primaryColor
    }
    
    
    func formatMessageContent(_ message: CraysMessage, colorScheme: ColorScheme) -> AttributedString {
        let isDark = colorScheme == .dark
            ...
            if let range = Range(matchRange, in: contentText) {
                if lastEndIndex < range.lowerBound {
                    ...
                    if !beforeText.isEmpty {
                        ...
                        normalStyle.font = .custom("Poppins-Regular", size: 14)
                ...
                matchStyle.font = .custom("Poppins-SemiBold", size: 14)
                
                if matchType == "mention" {
                    matchStyle.foregroundColor = Color.red
                } else {
                    // Hashtag
    ...
    @MainActor
    func formatMessageAsText(_ message: CraysMessage, colorScheme: ColorScheme) -> AttributedString {
        ...
        if message.sender != "system" {
            ...
            let fontWeight: String = isSelf ? "Poppins-Bold" : "Poppins-Medium"
            senderStyle.font = .custom(fontWeight, size: 14)

            // Make sender clickable: encode senderPeerID into a custom URL
            if let spid = message.senderPeerID, let url = URL(string: "Crays://user/\(spid.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? spid)") {
            ...
            if (content.count > 4000 || content.hasVeryLongToken(threshold: 1024)) && !containsCashuEarly {
                ...
                plainStyle.font = isSelf
                    ...
                    : .custom("Poppins-Regular", size: 14)
            ...
            for (range, type) in allMatches {
                ...
                if let nsRange = Range(range, in: content) {
                    ...
                    if type == "mention" {
                        ...
                        let fontWeight: String = isSelf ? "Poppins-Bold" : "Poppins-Medium"
                        mentionStyle.font = .custom(fontWeight, size: 14)
                        ...
                        if type == "hashtag" {
                            // If the hashtag is a valid geohash, make it tappable (Crays://geohash/<gh>)
                            ...
                            if isGeohash && !attachedToMention && standalone, let url = URL(string: "Crays://geohash/\(token)") {
                                tagStyle.link = url
                            ...
                            var matchStyle = AttributeContainer()
                            let fontWeight: String = isSelf ? "Poppins-Bold" : "Poppins-Medium"
                            matchStyle.font = .custom(fontWeight, size: 14)
            ...
            timestampStyle.font = .custom("Poppins-Regular", size: 10)
            ...
            contentStyle.font = .custom("Poppins-ThinItalic", size: 12)
    ...
    func formatMessage(_ message: CraysMessage, colorScheme: ColorScheme) -> AttributedString {
        var result = AttributedString()
        ...
        if message.sender == "system" {
            ...
            // Bold the user's own nickname
            let fontWeight: String = message.sender == nickname ? "Poppins-Bold" : "Poppins-Medium"
            senderStyle.font = .custom(fontWeight, size: 12)
            ...
            for match in matches {
                ...
                if let range = Range(match.range(at: 0), in: contentText) {
                    ...
                    var mentionStyle = AttributeContainer()
                    mentionStyle.font = .custom("Poppins-SemiBold", size: 14)
                    mentionStyle.foregroundColor = Color.red
            ...
            if message.isRelay, let originalSender = message.originalSender {
                ...
                relayStyle.font = .custom("Poppins-Regular", size: 11)
    ...
    @MainActor
    private func peerColor(for message: CraysMessage, isDark: Bool) -> Color {
        if let spid = message.senderPeerID {
            if spid.hasPrefix("nostr:") || spid.hasPrefix("nostr_") {
                let bare: String = {
                    if spid.hasPrefix("nostr:") { return String(spid.dropFirst(6)) }
    ...
    private func addMessage(_ message: CraysMessage) {
    ...
    private func addPrivateMessage(_ message: CraysMessage, for peerID: String) {
    ...
    func getPeer(byID peerID: String) -> CraysPeer? {
    ...
    func didReceiveMessage(_ message: CraysMessage) {
        Task { @MainActor in
            // Early validation
    ...
    func didUpdatePeerList(_ peers: [String]) {
        ...
        DispatchQueue.main.async {
            ...
            // Smart notification logic for "Craysters nearby"
            if !peers.isEmpty {
                ...
                if meshPeers.count > 0 && hadNone && !self.hasNotifiedNetworkAvailable {
                    ...
                    SecureLogger.log("👥 Sent Craysters nearby notification for \(meshPeers.count) mesh peers", 
                                   category: SecureLogger.session, level: .info)
                }
            } else {
    ...
    private func updateMessageDeliveryStatus(_ messageID: String, status: DeliveryStatus) {
        ...
        for (peerID, chatMessages) in privateChats {
            ...
            // Update delivery status directly (CraysMessage is a class/reference type)
    ...
    @MainActor
    private func handleNostrMessage(_ giftWrap: NostrEvent1) {
        ...
        do {
            ...
            // Expect embedded Crays packet content
            guard content.hasPrefix("Crays1:") else {
                SecureLogger.log("Ignoring non-embedded Nostr DM content", category: SecureLogger.session, level: .debug)
                return
            }

            guard let packetData = Self.base64URLDecode(String(content.dropFirst("Crays1:".count))),
                  let packet = CraysPacket.from(packetData) else {
                SecureLogger.log("Failed to decode embedded Crays packet from Nostr DM", category: SecureLogger.session, level: .error)
                return
            }
    ...
    @MainActor
    private func isMessageBlocked(_ message: CraysMessage) -> Bool {
    ...
    private func processActionMessage(_ message: CraysMessage) -> CraysMessage {
    ...
    private func migratePrivateChatsIfNeeded(for peerID: String, senderNickname: String) {
        ...
        if privateChats[peerID] == nil || privateChats[peerID]?.isEmpty == true {
            var migratedMessages: [CraysMessage] = []
    ...
    @MainActor
    private func handlePrivateMessage(_ message: CraysMessage) {
    ...
    @MainActor
    private func handlePublicMessage(_ message: CraysMessage) {
    ...
    @MainActor
    private func enqueuePublic(_ message: CraysMessage) {
...
private func checkForMentions(_ message: CraysMessage) {
...
/// Send haptic feedback for special messages (iOS only)
    private func sendHapticFeedback(for message: CraysMessage) {
        #if os(iOS)
