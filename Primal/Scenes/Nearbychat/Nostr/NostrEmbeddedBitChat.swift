...
// MARK: - Crays-over-Nostr Adapter

struct NostrEmbeddedCrays {
    /// Build a `Crays1:` base64url-encoded Crays packet carrying a private message for Nostr DMs.
    static func encodePMForNostr(content: String, messageID: String, recipientPeerID: String, senderPeerID: String) -> String? {
        ...
        return "Crays1:" + base64URLEncode(data)
    }

    /// Build a `Crays1:` base64url-encoded Crays packet carrying a delivery/read ack for Nostr DMs.
    ...
    /// Build a `Crays1:` ACK (delivered/read) without an embedded recipient peer ID (geohash DMs).
    ...
    /// Build a `Crays1:` payload without an embedded recipient peer ID (used for geohash DMs).
