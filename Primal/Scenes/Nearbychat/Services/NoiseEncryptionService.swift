...
/// peer-to-peer communication in Crays. Acts as the bridge between the transport
...
/// Manages end-to-end encryption for Crays using the Noise Protocol Framework.
...
class NoiseEncryptionService {
    ...
    // Thread safety
    private let serviceQueue = DispatchQueue(label: "chat.Crays.noise.service", attributes: .concurrent)
    ...
    private func canonicalAnnounceBytes(peerID: Data, noiseKey: Data, ed25519Key: Data, nickname: String, timestampMs: UInt64) -> Data {
        var out = Data()
        // context
        let context = "Crays-announce-v1".data(using: .utf8) ?? Data()
    ...
    /// Sign a CraysPacket using the noise private key
    func signPacket(_ packet: CraysPacket) -> CraysPacket? {
    ...
    /// Verify a CraysPacket signature using the provided public key
    func verifyPacketSignature(_ packet: CraysPacket, publicKey: Data) -> Bool {
...
/// These types wrap the underlying Crays protocol messages with encryption metadata.
