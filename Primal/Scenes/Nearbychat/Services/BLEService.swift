...
/// - Emits events exclusively via `CraysDelegate` for UI.
...
final class BLEService: NSObject {
    ...
    private var recentAnnounceBySender: [String: CraysPacket] = [:]
    ...
    private var pendingDirectedRelays: [String: [String: (packet: CraysPacket, enqueuedAt: Date)]] = [:]
    ...
    private func makeMessageID(for packet: CraysPacket) -> String {
    ...
    private func sendEncrypted(_ packet: CraysPacket, data: Data, pad: Bool) {
    ...
    private func sendGenericBroadcast(_ packet: CraysPacket, data: Data, pad: Bool) {
        sendOnAllLinks(packet: packet, data: data, pad: pad, directedOnlyPeer: nil)
    }

    private func sendOnAllLinks(packet: CraysPacket, data: Data, pad: Bool, directedOnlyPeer: String?) {
    ...
    private func spoolDirectedPacket(_ packet: CraysPacket, recipientPeerID: String) {
    ...
    private func flushDirectedSpool() {
        ...
        let toSend: [(String, CraysPacket)] = collectionsQueue.sync(flags: .barrier) {
            var out: [(String, CraysPacket)] = []
            let now = Date()
    ...
    private func rebroadcastRecentAnnounces() {
        ...
        let packets: [CraysPacket] = collectionsQueue.sync {
    ...
    private func sendFragmentedPacket(_ packet: CraysPacket, pad: Bool, maxChunk: Int? = nil, directedOnlyPeer: String? = nil) {
    ...
    private func handleReceivedPacket(_ packet: CraysPacket, from peerID: String) {
    ...
    private func handleAnnounce(_ packet: CraysPacket, from peerID: String) {
    ...
    private func handleMessage(_ packet: CraysPacket, from peerID: String) {
    ...
    private func handleNoiseHandshake(_ packet: CraysPacket, from peerID: String) {
           ...
           recipientID.hexEncodedString() == myPeerID {
            ...
            do {
                if let response = try noiseService.processHandshakeMessage(from: peerID, message: packet.payload) {
                    // Send response
                    let responsePacket = CraysPacket(
                        type: MessageType.noiseHandshake.rawValue,
                        senderID: Data(hexString: myPeerID) ?? Data(),
                        recipientID: Data(hexString: peerID),
                        timestamp: UInt64(Date().timeIntervalSince1970 * 1000),
    ...
    private func handleNoiseEncrypted(_ packet: CraysPacket, from peerID: String) {
    ...
    private func handleLeave(_ packet: CraysPacket, from peerID: String) {
    ...
    private func performCleanup() {
        ...
        collectionsQueue.async(flags: .barrier) { [weak self] in
            ...
            if !self.pendingDirectedRelays.isEmpty {
                var cleaned: [String: [String: (packet: CraysPacket, enqueuedAt: Date)]] = [:]
...
extension BLEService {
    func _test_handlePacket(_ packet: CraysPacket, fromPeerID: String) {
...
extension BLEService: CBPeripheralDelegate {
    ...
    func peripheral(_ peripheral: CBPeripheral, didModifyServices invalidatedServices: [CBService]) {
        ...
        if !hasOurService {
            ...
            SecureLogger.log("❌ Crays service removed - disconnecting from \(peripheral.name ?? peripheral.identifier.uuidString)", category: SecureLogger.session, level: .warning)
            centralManager?.cancelPeripheralConnection(peripheral)
        } else {
            // Try to rediscover
            peripheral.discoverServices([BLEService.serviceUUID])
