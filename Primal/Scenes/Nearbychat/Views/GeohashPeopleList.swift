            ...
            VStack(alignment: .leading, spacing: 0) {
                ForEach(finalOrder.filter { personByID[$0] != nil }, id: \.self) { pid in
                    ...
                    HStack(spacing: 4) {
                        ...
                        let rowColor: Color = isMe ? .red : assignedColor
                        Image(systemName: icon).font(.custom("Poppins-Regular", size: 12)).foregroundColor(rowColor)
                        ...
                        HStack(spacing: 0) {
                            ...
                            if !suffix.isEmpty {
                                let suffixColor = isMe ? Color.black.opacity(0.6) : rowColor.opacity(0.6)
                                Text(suffix)
                                    .font(.custom("Poppins-Regular", size: 14))
