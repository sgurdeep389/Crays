...
struct MeshPeerList: View {
    ...
    var body: some View {
        if viewModel.allPeers.isEmpty {
            ...
            let mapped: [(peer: CraysPeer, isMe: Bool, hasUnread: Bool, enc: EncryptionStatus)] = viewModel.allPeers.map { peer in
            ...
            let peers: [(peer: CraysPeer, isMe: Bool, hasUnread: Bool, enc: EncryptionStatus)] = displayIDs.compactMap { id in
            ...
            VStack(alignment: .leading, spacing: 0) {
                ForEach(0..<peers.count, id: \.self) { idx in
                    ...
                    HStack(spacing: 4) {
                        ...
                        let baseColor = isMe ? Color.red : assigned
                        if isMe {
                        ...
                        HStack(spacing: 0) {
                            Text(base)
                                ...
                                .foregroundColor(baseColor)
                            if !suffix.isEmpty {
                                let suffixColor = isMe ? Color.black.opacity(0.6) : baseColor.opacity(0.6)
                                Text(suffix)
                                    .font(.custom("Poppins-Regular", size: 14))
