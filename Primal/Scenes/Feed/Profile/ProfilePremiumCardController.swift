...
class ProfilePremiumCardController: UIViewController {
    ...
    lazy var secondLabel = UILabel((user.data.secondIdentifier ?? "").replacingOccurrences(of: "primal", with: "crays"), color: .white, font: .appFont(withSize: 14, weight: .regular))
    ...
    func setup() {
        ...
        if let custom = PremiumCustomizationManager.instance.getPremiumInfo(pubkey: user.data.pubkey) {
            ...
            if custom.tier == "premium-legend" {
                ...
                let formatter = DateFormatter()
                formatter.dateFormat = "MMMM d, yyyy"
                sinceLabel.text = "\(custom.cohort_1.replacingOccurrences(of: "Primal", with: "Crays")) since \(formatter.string(from: Date(timeIntervalSince1970: custom.premium_since)))"
                 
                otherLegendsButton.configuration = .coloredButton("See other Crays OGs", color: .accent)
                becomeLegendParent.alpha = 0.001
                becomeLegendParent.isUserInteractionEnabled = false
            }
        }
