...
final class MenuContainerController: UIViewController, Themeable {
    ...
    lazy var viewsToTranslate = [notificationIndicator, mainStack]
...
private extension MenuContainerController {
    func setup() {
        ...
        let accounts = MenuItemButton(title: "ADD ACCOUNTS", image: .addAccount)
        let wallet = MenuItemButton(title: "Wallet", image: .tabIconWallet1)
        let supportus = MenuItemButton(title: "Founders Club", image: .support)
        ...
        let buttonsStack = UIStackView(arrangedSubviews: [profile, messages, premium,settings,supportus,bookmarks, redeemCode,wallet,accounts,signOut])
        [
        ...
        messages.addAction(.init(handler: { [unowned self] _ in showViewController(MessagesViewController(titleLbl: "Messages")) }), for: .touchUpInside)
        bookmarks.addAction(.init(handler: { [unowned self] _ in showViewController(PublicBookmarksViewController()) }), for: .touchUpInside)
        premium.addAction(.init(handler: { [unowned self] _ in
            mainTabBarController?.setTabBarHidden(true, animated: true)
            showViewController(PremiumSearchNameController(title: "Find Crays Name", callback: { name in
                self.showViewController(PremiumBuySubscriptionController(pickedName: name, kind: .premium, state: .onboardingFinish))
            }))
//            showViewController(PremiumViewController())
        }), for: .touchUpInside)
        ...
        accounts.addTarget(self, action: #selector(accountsButtonPressed), for: .touchUpInside)
        
        wallet.addTarget(self, action: #selector(walletButtonPressed), for: .touchUpInside)
        
        supportus.addTarget(self, action: #selector(supportButtonPressed), for: .touchUpInside)
    ...
    func update(_ user: ParsedUser) {
        ...
        if user.displayName.isEmpty {
            if CheckNip05Manager.instance.isVerified(user) {
                nameLabel.text = user.parsedNip.replacingOccurrences(of: "primal", with: "crays")
            } else {
            ...
            if CheckNip05Manager.instance.isVerified(user) {
                domainLabel.text = user.parsedNip.replacingOccurrences(of: "primal", with: "crays")
            } else {
    ...
    @objc func supportButtonPressed() {
        self.showViewController(WebViewController(urlString: "https://preview--crays-support-hub.lovable.app", title: "Founders Club"))
    }
