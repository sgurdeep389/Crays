...
extension UIViewController {
    ...
    func customSearchButton1(scope: SearchScope = .global, type: SearchType = .notes) -> UIBarButtonItem {
        let view = UIView().constrainToSize(70)
        let icon = UIImageView(image: UIImage(named: "navSearch"))
        icon.tintColor = .foreground
        view.addSubview(icon)
        icon.centerToSuperview(axis: .vertical).pinToSuperview(edges: .trailing)
        ...
        let icon1 = UIImageView(image: .selectedTabIconNotifications)
        icon1.tintColor = .foreground
        view.addSubview(icon1)
        icon1.centerToSuperview(axis: .vertical).pinToSuperview(edges: .leading)
        let button1 = UIButton()
        view.addSubview(button1)
        button1.pinToSuperview()
