...
final class PrimalSchemeDeeplinkHandler: DeeplinkHandlerProtocol {
    ...
    func openURL(_ url: URL) {
        ...
        if host == "sharedImages" {
            ...
            let fileNames: [String] = UserDefaults(suiteName: "group.chat.Crays")?.value(forKey: "sharedImageNames") as? [String] ?? []
            let fileManager = FileManager.default
            RootViewController.instance.navigateTo = .newPost(text: text, files: fileNames.compactMap {
                guard let containerURL = fileManager.containerURL(forSecurityApplicationGroupIdentifier: "group.chat.Crays") else { return nil }
