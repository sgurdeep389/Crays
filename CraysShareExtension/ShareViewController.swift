...
final class ShareViewController: UIViewController {
    ...
    private func saveAndFinish(url: URL, title: String?) {
        ...
        if let json = try? JSONSerialization.data(withJSONObject: payload),
           let s = String(data: json, encoding: .utf8) {
            ...
            finishWithMessage("✓ Shared link to Crays")
        } else {
    ...
    private func saveAndFinish(text: String) {
        ...
        finishWithMessage("✓ Shared text to Crays")
    }

    private func saveToSharedDefaults(content: String, type: String) {
        guard let userDefaults = UserDefaults(suiteName: "group.chat.Crays") else { return }
