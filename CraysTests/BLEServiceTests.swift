...
final class BLEServiceTests: XCTestCase {
    ...
    func testSendPublicMessage() {
        ...
        let delegate = MockCraysDelegate { message in
    ...
    func testSimulateIncomingMessage() {
        ...
        let incomingMessage = CraysMessage(
            id: "MSG456",
            sender: "RemoteUser",
    ...
    func testMessageDeliveryHandler() {
        ...
        service.packetDeliveryHandler = { packet in
            if let msg = CraysMessage.fromBinaryPayload(packet.payload) {
...
private class MockCraysDelegate: CraysDelegate {
    private let messageHandler: (CraysMessage) -> Void
    
    init(_ handler: @escaping (CraysMessage) -> Void) {
