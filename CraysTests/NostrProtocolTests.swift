...
final class NostrProtocolTests: XCTestCase {
    ...
    func testAckRoundTripNIP44V2_Delivered() throws {
        ...
        guard let embedded = NostrEmbeddedCrays.encodeAckForNostrNoRecipient(type: .delivered, messageID: messageID, senderPeerID: senderPeerID) else {
        ...
        // Parse Crays payload
        XCTAssertTrue(content.hasPrefix("Crays1:"))
        let base64url = String(content.dropFirst("Crays1:".count))
        guard let packetData = Self.base64URLDecode(base64url),
              let packet = CraysPacket.from(packetData) else {
            return XCTFail("Failed to decode Crays packet")
        }
    ...
    func testAckRoundTripNIP44V2_ReadReceipt() throws {
        ...
        guard let embedded = NostrEmbeddedCrays.encodeAckForNostrNoRecipient(type: .readReceipt, messageID: messageID, senderPeerID: senderPeerID) else {
