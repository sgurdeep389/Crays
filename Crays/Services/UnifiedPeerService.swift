...
class UnifiedPeerService: ObservableObject, TransportPeerEventsDelegate {
    ...
    @Published private(set) var peers: [CraysPeer] = []
    @Published private(set) var connectedPeerIDs: Set<String> = []
    @Published private(set) var favorites: [CraysPeer] = []
    @Published private(set) var mutualFavorites: [CraysPeer] = []
    ...
    private func updatePeers() {
        ...
        enrichedPeers.sort { lhs, rhs in
            ...
            func rank(_ p: CraysPeer) -> Int { p.isConnected ? 2 : (p.isReachable ? 1 : 0) }
        ...
        var favoritesList: [CraysPeer] = []
        var mutualsList: [CraysPeer] = []
        var newIndex: [String: CraysPeer] = [:]
    ...
    /// Get peer by ID
    func getPeer(by id: String) -> CraysPeer? {
        return peerIndex[id]
    }
