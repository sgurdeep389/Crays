...
/// - **CraysDelegate**: Receives messages and events from BluetoothMeshService
...
class ChatViewModel: ObservableObject, CraysDelegate {
    ...
    @Published var allPeers: [CraysPeer] = []  // Unified peer list including favorites
    private var peerIndex: [String: CraysPeer] = [:] // Quick lookup by peer ID
    ...
    @Published var privateChats: [String: [CraysMessage]] = [:] // peerID -> messages
    ...
    init() {
        ...
        NotificationCenter.default.addObserver(
            ...
            name: Notification.Name("Crays.retryMessage"),
            object: nil
        )
    ...
    func updateBluetoothState(_ state: CBManagerState) {
        ...
        switch state {
        case .poweredOff:
            bluetoothAlertMessage = "Bluetooth is turned off. Please turn on Bluetooth in Settings to use Crays circle."
            showBluetoothAlert = true
        case .unauthorized:
            bluetoothAlertMessage = "Crays circle needs Bluetooth permission to connect with nearby devices. Please enable Bluetooth access in Settings."
            showBluetoothAlert = true
        case .unsupported:
            bluetoothAlertMessage = "This device does not support Bluetooth. Crays circle requires Bluetooth to function."
            showBluetoothAlert = true
        case .poweredOn:
    ...
    func getSenderColor(for message: CraysMessage, colorScheme: ColorScheme) -> Color {
        ...
        let primaryColor = isDark ? Color.black : Color.black
    ...
    func formatMessageAsText(_ message: CraysMessage, colorScheme: ColorScheme) -> AttributedString {
        ...
        timestampStyle.font = .custom("Poppins-Regular", size: 12)
        ...
        if message.sender != "system" {
            ...
            for (range, type) in allMatches {
                ...
                if let nsRange = Range(range, in: content) {
                    ...
                    if !beforeText.isEmpty {
                        ...
                        beforeStyle.font = .custom("Poppins-Regular", size: 14)

                        if isMentioned {
                            beforeStyle.font = .custom("Poppins-Bold", size: 14)
                        }
            ...
            if lastEnd < content.endIndex {
                ...
                remainingStyle.font = .custom("Poppins-Regular", size: 14)
                if isMentioned {
                    remainingStyle.font = .custom("Poppins-Bold", size: 14)
            ...
            contentStyle.font = .custom("Poppins-Italic", size: 12)
        ...
        case "/testnostr":
            ...
            Task { @MainActor in
                if let relayManager = self.nostrRelayManager {
                    ...
                    let completeMessage = CraysMessage(
                        sender: "system",
    ...
    func handleHandshakeRequest(from peerID: String, nickname: String, pendingCount: UInt8) {
        ...
        let notificationMessage = CraysMessage(
            sender: "system",
    ...
    func didReceiveMessage(_ message: CraysMessage) {
        ...
        if message.isPrivate {
            ...
            let finalMessage: CraysMessage
            if isActionMessage {
                // Convert to system message
                finalMessage = CraysMessage(
                    sender: "system",
