...
/// Represents a peer in the Crays network with all associated metadata
struct CraysPeer: Identifiable, Equatable {
    ...
    static func == (lhs: CraysPeer, rhs: CraysPeer) -> Bool {
        lhs.id == rhs.id
    }
}
...
class PeerManager: ObservableObject {
    @Published var peers: [CraysPeer] = []
    @Published var favorites: [CraysPeer] = []
    @Published var mutualFavorites: [CraysPeer] = []
    ...
    func updatePeers() {
        ...
        var favorites: [CraysPeer] = []
        var mutualFavorites: [CraysPeer] = []
    ...
    func toggleFavorite(_ peer: CraysPeer) {
        if peer.isFavorite {
