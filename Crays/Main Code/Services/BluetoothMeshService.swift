...
/// and implements the Crays mesh networking protocol.
///
/// ## Overview
/// This service is the heart of Crays's decentralized architecture. It manages all
...
/// - **Central Mode**: Scans for and connects to other Crays devices
...
/// - Clear error reporting through CraysDelegate
///
/// ## Usage Example
/// ```swift
...
/// Manages all Bluetooth LE networking operations for the Crays mesh network.
...
class BluetoothMeshService: NSObject {
    ...
    private let collectionsQueue = DispatchQueue(label: "Crays.collections", attributes: .concurrent)
    ...
    weak var delegate: CraysDelegate?
    ...
    private let messageQueue = DispatchQueue(label: "Crays.messageQueue", attributes: .concurrent) // Concurrent queue with barriers
    ...
    private static let centralManagerRestorationID = "com.Crays.central"
    private static let peripheralManagerRestorationID = "com.Crays.peripheral"
    ...
    private let connectionStateQueue = DispatchQueue(label: "chat.Crays.connectionState", attributes: .concurrent)
    
    // MARK: - Protocol ACK Tracking
    
    // Protocol-level ACK tracking
    private var pendingAcks: [String: (packet: CraysPacket, timestamp: Date, retries: Int)] = [:]
    ...
    // Message aggregation
    private var pendingMessages: [(message: CraysPacket, destination: String?)] = []
    ...
    private func flushPendingMessages() {
        ...
        messageQueue.async { [weak self] in
            ...
            var messagesByDestination: [String?: [CraysPacket]] = [:]
    ...
    func sendBroadcastAnnounce() {
        ...
        let announcePacket = CraysPacket(
    ...
    private func getEncryptionQueue(for peerID: String) -> DispatchQueue {
        ...
        let queue = DispatchQueue(label: "Crays.encryption.\(peerID)", qos: .userInitiated)
    ...
    private func cacheMessage(_ packet: CraysPacket, messageID: String) {
    ...
    private func broadcastPacket(_ packet: CraysPacket) {
        ...
        guard let data = packet.toBinaryData() else { 
            ...
            if senderID == self.myPeerID,
               ...
               let message = CraysMessage.fromBinaryPayload(packet.payload) {
    ...
    private func handleReceivedPacket(_ packet: CraysPacket, from peerID: String, peripheral: CBPeripheral? = nil) {
        messageQueue.async(flags: .barrier) { [weak self] in
            guard let self = self else { return }
        ...
        case .message:
            ...
            if let recipientID = packet.recipientID {
                if recipientID == SpecialRecipients.broadcast {
                    ...
                    if let message = CraysMessage.fromBinaryPayload(packet.payload) {
                        ...
                        let messageWithPeerID = CraysMessage(
                    ...
                    // Parse the message
                    if let message = CraysMessage.fromBinaryPayload(decryptedPayload) {
    ...
    private func sendFragmentedPacket(_ packet: CraysPacket) {
        ...
        for (index, fragmentData) in fragments.enumerated() {
            ...
            let fragmentPacket = CraysPacket(
    ...
    private func handleFragment(_ packet: CraysPacket, from peerID: String) {
        // Handling fragment
        ...
        if let fragments = incomingFragments[fragmentID],
           fragments.count == total {
            ...
            if let reassembledPacket = CraysPacket.from(reassembledData) {
                // Clean up
...
extension BluetoothMeshService: CBPeripheralDelegate {
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        guard let packet = CraysPacket.from(data) else { 
...
extension BluetoothMeshService: CBPeripheralManagerDelegate {
    ...
    func peripheralManager(_ peripheral: CBPeripheralManager, didReceiveWrite requests: [CBATTRequest]) {
        ...
        for request in requests {
            if let data = request.value {
                ...
                if let packet = CraysPacket.from(data) {
                    let peerID = packet.senderID.hexEncodedString()
    ...
    private func scheduleRelay(_ packet: CraysPacket, messageID: String, delay: TimeInterval) {
        pendingRelaysLock.lock()
    ...
    private func handleNoiseEncryptedMessage(from peerID: String, encryptedData: Data, originalPacket: CraysPacket, peripheral: CBPeripheral? = nil) {
        // Use noiseService directly
        ...
        do {
            ...
            if let innerPacket = CraysPacket.from(decryptedData) {
    ...
    private func sendProtocolAck(for packet: CraysPacket, to peerID: String, hopCount: UInt8 = 0) {
    ...
    private func sendProtocolNack(for packet: CraysPacket, to peerID: String, reason: String, errorCode: ProtocolNack.ErrorCode) {
        let packetID = generatePacketID(for: packet)
    ...
    private func generatePacketID(for packet: CraysPacket) -> String {
    ...
    private func trackPacketForAck(_ packet: CraysPacket) {
        let packetID = generatePacketID(for: packet)
    ...
    private func sendDirectToRecipient(_ packet: CraysPacket, recipientPeerID: String) -> Bool {
    ...
    private func sendHandshakeRequest(to targetPeerID: String, pendingCount: UInt8) {
        ...
        let packet = CraysPacket(type: MessageType.handshakeRequest.rawValue,
                                  ttl: 6,
                                  senderID: myPeerID,
                                  payload: requestData)
    ...
    private func sendViaSelectiveRelay(_ packet: CraysPacket, recipientPeerID: String) {
