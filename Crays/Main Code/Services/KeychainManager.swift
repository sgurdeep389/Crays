...
class KeychainManager {
    ...
    private let service = "chat.Crays"
    private let appGroup = "group.chat.Crays"
    ...
    private func cleanupLegacyKeychainItems() {
        ...
        let cleanupKey = "Crays.keychain.cleanup.v2"
        ...
        let legacyServices = [
            "com.Crays.passwords",
            "com.Crays.deviceidentity",
            "com.Crays.noise.identity",
            "chat.Crays.passwords",
            "Crays.keychain"
        ]
    ...
    private func isSandboxed() -> Bool {
        ...
        let testPath = homeDir.appendingPathComponent("../../../tmp/Crays_sandbox_test_\(UUID().uuidString)")
    ...
    private func saveData(_ data: Data, forKey key: String) -> Bool {
        ...
        query[kSecAttrLabel as String] = "Crays-\(key)"
    ...
    func resetCleanupFlag() {
        UserDefaults.standard.removeObject(forKey: "Crays.keychain.cleanup.v2")
    }
    ...
    func aggressiveCleanupLegacyItems() -> Int {
        ...
        // List of KNOWN Crays service names from our development history
        let knownCraysServices = [
            "com.Crays.passwords",
            "com.Crays.deviceidentity",
            "com.Crays.noise.identity",
            "chat.Crays.passwords",
        ...
        for legacyService in knownCraysServices {
            let deleteQuery: [String: Any] = [
                kSecClass as String: kSecClassGenericPassword,
        ...
        // Now search for items that have our specific account patterns with Crays service names
        let searchQuery: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecMatchLimit as String: kSecMatchLimitAll,
            kSecReturnAttributes as String: true
        ...
        if status == errSecSuccess, let items = result as? [[String: Any]] {
            for item in items {
                ...
                // ONLY delete if service name contains "Crays" somewhere
                // This ensures we never touch other apps' keychain items
                var shouldDelete = false
                
                // Check if service contains "Crays" (case insensitive) but NOT our current service
                let serviceLower = service.lowercased()
                if service != self.service && serviceLower.contains("Crays") {
                    shouldDelete = true
                }
